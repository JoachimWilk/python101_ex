...
def otworz(plik):
    if os.path.isfile(plik):   #czy isteniej plik słownika?
        with open(plik, "r") as sTxt:  # otwó©z plik do odczytu
...
# zmienna oznaczająca, że uzytkownik uzupełnił lub zmienił słownik
nowy = False
...
while True:
    ...
    if wobcy == "koniec":
        break
    elif dane.count(":") == 1:  # sprawdzamy poprawaność wprowadzonych danych
        if wobcy in slownik:
        ...
        if wobcy not in slownik or op == "t":
            znaczenia = t[1].split(",") # podane znaczenia zpisujemy w liście
